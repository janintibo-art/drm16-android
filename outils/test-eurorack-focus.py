        ...
        for largeur, hauteur in FORMATS:
            ...
            result=pg.evaluate("""() => {
              ...
              Object.keys(EUR_CAT).forEach((type,i)=>{
                ...
                r.push({type,kn:(type === "stutterlive" ? root.querySelectorAll(".stl-grand select").length===4 && !!root.querySelector(".stl-hold") && !!root.querySelector(".stl-dose input") : type === "harmonie8" ? root.querySelectorAll(".hr8-case").length===8 && root.querySelectorAll(".hr8-grand select").length===11 : type === "kicklab" || type === "ducktrig" ? root.querySelectorAll(".kb-range").length===d.kns.length && !!root.querySelector(".kb-courbe") : type === "break32" ? root.querySelectorAll(".br32-pas").length===16 && root.querySelectorAll(".br32-editeur select[data-champ]").length===8 && !!root.querySelector(".brs-source") : type === "scenes8" ? root.querySelectorAll(".sc8-scene").length===8 && root.querySelectorAll(".sc8-editeur select").length===9 : type === "melo32" ? root.querySelectorAll(".ml32-pas").length===16 && root.querySelectorAll(".ml32-global select").length===6 : d.interface ? root.querySelectorAll('.dr32-pas').length===16 && root.querySelectorAll('.dr32-piste').length===4 && root.querySelectorAll('.dr32-reglages select').length===4 : root.querySelectorAll('[role="slider"]').length===d.kns.length),
