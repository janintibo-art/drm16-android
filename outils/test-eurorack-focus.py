...
def main():
    ...
    with sync_playwright() as p:
        ...
        for largeur, hauteur in FORMATS:
            ...
            # 105 graphes audio. Aucun état utilisateur réel n’est employé.
            result=pg.evaluate("""() => {
              const r=[];
              Object.keys(EUR_CAT).forEach((type,i)=>{
                ...
                r.push({type,kn:(type === "melo32" ? root.querySelectorAll(".ml32-pas").length===16 && root.querySelectorAll(".ml32-global select").length===6 : d.interface ? root.querySelectorAll('.dr32-pas').length===16 && root.querySelectorAll('.dr32-piste').length===4 && root.querySelectorAll('.dr32-reglages select').length===4 : root.querySelectorAll('[role="slider"]').length===d.kns.length),
                  j:root.querySelectorAll('.ef-jack').length===d.jacks.length,bad:bad.length,
                  deborde:zone.scrollWidth-zone.clientWidth});
              });return r;
            }""")
            verifier(len(result)==105 and all(x['kn'] and x['j'] and x['bad']==0 and x['deborde']<=1 for x in result),prefixe+' : 105 façades complètes et cibles confortables : '+str([x for x in result if not(x['kn'] and x['j']) or x['bad'] or x['deborde']>1]))
            ...
            print('Contrôlé : '+prefixe+' · gestes, audio, câbles, cycle de vie et 105 types',flush=True)
            ctx.close()
        nav.close()
