...
def main():
    ...
    with sync_playwright() as p:
        ...
        for largeur, hauteur in FORMATS:
            ...
            # 111 graphes audio. Aucun état utilisateur réel n’est employé.
            result=pg.evaluate("""() => {
              const r=[];
              Object.keys(EUR_CAT).forEach((type,i)=>{
                ...
                r.push({type,kn:(type === "kicklab" || type === "ducktrig" ? root.querySelectorAll(".kb-range").length===d.kns.length && !!root.querySelector(".kb-courbe") : type === "break32" ? root.querySelectorAll(".br32-pas").length===16 && root.querySelectorAll(".br32-editeur select[data-champ]").length===8 && !!root.querySelector(".brs-source") : type === "scenes8" ? root.querySelectorAll(".sc8-scene").length===8 && root.querySelectorAll(".sc8-editeur select").length===8 : type === "melo32" ? root.querySelectorAll(".ml32-pas").length===16 && root.querySelectorAll(".ml32-global select").length===6 : d.interface ? root.querySelectorAll('.dr32-pas').length===16 && root.querySelectorAll('.dr32-piste').length===4 && root.querySelectorAll('.dr32-reglages select').length===4 : root.querySelectorAll('[role="slider"]').length===d.kns.length),
                  j:root.querySelectorAll('.ef-jack').length===d.jacks.length,bad:bad.length,
                  deborde:zone.scrollWidth-zone.clientWidth});
              });return r;
            }""")
            verifier(len(result)==111 and all(x['kn'] and x['j'] and x['bad']==0 and x['deborde']<=1 for x in result),prefixe+' : 111 façades complètes et cibles confortables : '+str([x for x in result if not(x['kn'] and x['j']) or x['bad'] or x['deborde']>1]))
            ...
            print('Contrôlé : '+prefixe+' · gestes, audio, câbles, cycle de vie et 111 types',flush=True)
            ctx.close()
        nav.close()
