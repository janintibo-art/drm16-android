        ...
        for largeur, hauteur in FORMATS:
            ...
            result=pg.evaluate("""() => {
              ...
              Object.keys(EUR_CAT).forEach((type,i)=>{
                ...
                r.push({type,kn:(type === "break32" ? root.querySelectorAll(".br32-pas").length===16 && root.querySelectorAll(".br32-editeur select[data-champ]").length===8 && !!root.querySelector(".brs-source") : type === "scenes8" ? root.querySelectorAll(".sc8-scene").length===8 && root.querySelectorAll(".sc8-editeur select").length===8 : type === "melo32" ? root.querySelectorAll(".ml32-pas").length===16 && root.querySelectorAll(".ml32-global select").length===6 : d.interface ? root.querySelectorAll('.dr32-pas').length===16 && root.querySelectorAll('.dr32-piste').length===4 && root.querySelectorAll('.dr32-reglages select').length===4 : root.querySelectorAll('[role="slider"]').length===d.kns.length),
