...
def main():
    ...
    with sync_playwright() as p:
        ...
        for largeur, hauteur in FORMATS:
            ...
            # 112 graphes audio. Aucun état utilisateur réel n’est employé.
            result=pg.evaluate("""() => {
              const r=[];
            ...
            verifier(len(result)==112 and all(x['kn'] and x['j'] and x['bad']==0 and x['deborde']<=1 for x in result),prefixe+' : 112 façades complètes et cibles confortables : '+str([x for x in result if not(x['kn'] and x['j']) or x['bad'] or x['deborde']>1]))
            ...
            print('Contrôlé : '+prefixe+' · gestes, audio, câbles, cycle de vie et 112 types',flush=True)
            ctx.close()
        nav.close()
