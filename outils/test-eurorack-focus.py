...
def main():
    ...
    with sync_playwright() as p:
        ...
        for largeur, hauteur in FORMATS:
            ...
            # 120 graphes audio. Aucun état utilisateur réel n’est employé.
            result=pg.evaluate("""() => {
              const r=[];
              Object.keys(EUR_CAT).forEach((type,i)=>{
                ...
                r.push({type,kn:(type === "stutterlive" ? root.querySelectorAll(".stl-grand select").length===4 && !!root.querySelector(".stl-hold") && !!root.querySelector(".stl-dose input") : type === "harmonie8" ? root.querySelectorAll(".hr8-case").length===8 && root.querySelectorAll(".hr8-grand select").length===11 : type === "kicklab" || type === "ducktrig" ? root.querySelectorAll(".kb-range").length===d.kns.length && !!root.querySelector(".kb-courbe") : type === "break32" ? root.querySelectorAll(".br32-pas").length===16 && root.querySelectorAll(".br32-editeur select[data-champ]").length===8 && !!root.querySelector(".brs-source") : type === "scenes8" ? root.querySelectorAll(".sc8-scene").length===8 && root.querySelectorAll(".sc8-editeur select").length===9 : type === "melo32" ? root.querySelectorAll(".ml32-pas").length===16 && root.querySelectorAll(".ml32-global select").length===6 : type === "poly4" ? root.querySelectorAll(".p4-touche").length===13 && root.querySelectorAll(".p4-mem").length===4 && root.querySelectorAll(".p4-reglages select").length===7 && !!root.querySelector(".p4-memoriser") : d.interface ? root.querySelectorAll('.dr32-pas').length===16 && root.querySelectorAll('.dr32-piste').length===4 && root.querySelectorAll('.dr32-reglages select').length===4 : root.querySelectorAll('[role="slider"]').length===d.kns.length),
                  j:root.querySelectorAll('.ef-jack').length===d.jacks.length,bad:bad.length,
                  deborde:zone.scrollWidth-zone.clientWidth});
              });return r;
            }""")
            verifier(len(result)==120 and all(x['kn'] and x['j'] and x['bad']==0 and x['deborde']<=1 for x in result),prefixe+' : 120 façades complètes et cibles confortables : '+str([x for x in result if not(x['kn'] and x['j']) or x['bad'] or x['deborde']>1]))
            ...
            print('Contrôlé : '+prefixe+' · gestes, audio, câbles, cycle de vie et 120 types',flush=True)
            ctx.close()
        nav.close()
