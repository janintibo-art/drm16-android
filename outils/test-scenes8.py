...
def main():
    ...
    with sync_playwright() as p:
        ...
        for w,h in FORMATS:
            ...
            v(pg.evaluate('Object.keys(EUR_CAT).length===117 && EUR_ORDRE.filter(k=>k==="scenes8").length===1'),nom+' catalogue 117 sans doublon')
            v(pg.evaluate('EUR_MONTAGES.length===63 && EUR_MONTAGES.filter(p=>p.fam==="avance").length===8'),nom+' 63 montages dont huit avancés')
