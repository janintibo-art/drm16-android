...
def main():
    ...
    with sync_playwright() as p:
        ...
        for w,h in FORMATS:
            ...
            v(pg.evaluate('Object.keys(EUR_CAT).length===106 && EUR_ORDRE.filter(k=>k==="scenes8").length===1'),nom+' catalogue 106 sans doublon')
            v(pg.evaluate('EUR_MONTAGES.length===38 && EUR_MONTAGES.filter(p=>p.fam==="avance").length===6'),nom+' 36 anciens montages et deux nouveaux')
