...
def main():
    ...
    with sync_playwright() as p:
        ...
        for w,h in FORMATS:
            ...
            v(pg.evaluate('Object.keys(EUR_CAT).length===113 && EUR_ORDRE.filter(x=>x==="drum32").length===1'),nom+' catalogue 113 modules sans doublon')
            v(pg.evaluate('EUR_MONTAGES.length===56 && EUR_MONTAGES.filter(p=>p.fam==="ensemble").length===8'),nom+' 56 montages dont huit ensembles')
