...
test('40 paramètres et 11 prises uniques, aucune source audio',()=>{const e=env(),m=e.mod(),d=e.b.EUR_CAT.harmonie8;eq(d.kns.length,40);eq(new Set(d.kns.map(k=>k[0])).size,40);eq(new Set(d.jacks.map(j=>j[0])).size,11);eq(Object.keys(m.io.e),['in','rst']);eq(Object.keys(m.io.s),d.jacks.filter(j=>j[2]).map(j=>j[0]));eq(e.b.EUR_ORDRE,['harmonie8']);});
...
test('CYCLES LIBRES : 18 pas (9/8) au lieu de 16',()=>{const m=env().mod({pasmes:18});tic(m,0);for(let i=1;i<18;i++)eq(tic(m,i),['clk']);eq(tic(m,18),['change','clk']);eq(m.harmonie8.pos,1);});
test('CYCLES LIBRES : ignoré en mode SUIVI (SCÈNES)',()=>{const m=env().mod({sync:1,pasmes:20,len:3});for(let i=0;i<12;i++){tic(m,i);eq(m.harmonie8.pos,i%3);}});
