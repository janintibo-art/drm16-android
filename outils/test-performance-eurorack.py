...
def main():
 ...
 with sync_playwright() as p:
  ...
  for w,h in FORMATS:
   ...
   v(pg.evaluate('EUR_MONTAGES.length===68&&Object.keys(EUR_CAT).length===120'),tag+' catalogue 120 modules / 68 montages')
   ...
   pg.evaluate('basse.p.sub=.417;window.avantMemoire=JSON.stringify(EUR.mods.map(m=>m.p))');pg.locator('#ep-memoriser-a').click()
   v(pg.locator('#ep-rappeler-a').is_enabled(),tag+' point de retour créé')
   v(pg.evaluate('JSON.stringify(EUR.mods.map(m=>m.p))===avantMemoire'),tag+' mémoriser ne change pas les paramètres')
   pg.evaluate('M.regler(4,.95);basse.p.sub=.231;EUR_PERF_UI.actualiser()');accord['oui']=False;pg.locator('#ep-rappeler-a').click()
   v(pg.evaluate('M.courant().commandes[4].valeur===.95'),tag+' rappel refusé sans effet')
   accord['oui']=True;pg.locator('#ep-rappeler-a').click()
   ...
   v(pg.evaluate('JSON.stringify(EUR.mods.map(m=>m.p))===sonAvant&&!M.courant().memoireA'),tag+' nouvelle affectation silencieuse, ancien retour invalidé')
   ...
   pg.locator('#ep-memoriser-a').click();pg.evaluate('window.sauve=JSON.stringify(rackCourant());memEur();writeMem()')
   ...
   pg.locator('#eur-performance-ouvrir').click();v(pg.evaluate('M.courant().memoireA!==null&&M.courant().commandes[0].cibles.length===3'),tag+' restauration des affectations et mémoire')
   ...
   # PERFORMANCE MOUVANTE (v292) : point de retour B distinct de A, glissade
   # TRANSITION entre les deux et geste enregistré puis rejoué en boucle.
   pg.evaluate('M.regler(4,.8)');accord['oui']=True;pg.locator('#ep-memoriser-b').click()
   v(pg.locator('#ep-rappeler-b').is_enabled(),tag+' point de retour B créé')
   pg.evaluate('M.regler(4,.95)');pg.locator('#ep-rappeler-b').click()
   v(pg.evaluate('Math.abs(M.courant().commandes[4].valeur-.8)<.002'),tag+' rappel B distinct de A')
   pg.select_option('#ep-trans-duree','500');pg.locator('#ep-trans-a').click()
   v(pg.evaluate('M.transitionEnCours()'),tag+' transition démarrée vers A')
   pg.wait_for_function('!EUR_PERFORMANCE.transitionEnCours()',timeout=3000)
   v(pg.evaluate('Math.abs(M.courant().commandes[4].valeur-.2)<.02'),tag+' transition terminée sur la valeur A')
   pg.locator('#ep-trans-b').click();pg.locator('#ep-trans-stop').click()
   v(pg.evaluate('!M.transitionEnCours()'),tag+' arrêt manuel de la transition')
   pg.locator('#ep-geste-enr').click();v(pg.evaluate('M.enregistrementActif()'),tag+' enregistrement de geste démarré')
   dial=pg.locator('.ep-dial').nth(6);dial.scroll_into_view_if_needed();dial.hover();pg.mouse.wheel(0,-60)
   pg.wait_for_timeout(120);pg.locator('#ep-geste-enr').click()
   v(pg.evaluate('!!M.courant().geste'),tag+' geste enregistré')
   pg.locator('#ep-geste-lecture').click();v(pg.evaluate('M.gesteEnLecture()'),tag+' lecture du geste démarrée')
   pg.locator('#ep-geste-lecture').click();v(pg.evaluate('!M.gesteEnLecture()'),tag+' lecture du geste arrêtée')
   accord['oui']=True;pg.locator('#ep-geste-effacer').click();v(pg.evaluate('!M.courant().geste'),tag+' geste effacé')
   ...
   v(pg.evaluate('M.courant().commandes[7].nom==="MONTÉE"&&!M.courant().memoireA'),tag+' second montage indépendant')
   ...
   v(pg.locator('#ep-memoriser-a').is_disabled(),tag+' aucune fausse mémoire sans cibles')
