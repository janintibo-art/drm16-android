...
def main():
 ...
 with sync_playwright() as p:
  ...
  for w,h in FORMATS:
   ...
   v(pg.evaluate('Object.keys(EUR_CAT).length===123 && new Set(EUR_ORDRE).size===123'),tag+' 123 modules uniques')
   v(pg.evaluate('EUR_MONTAGES.length===74 && EUR_MONTAGES.filter(p=>p.fam==="rave").length===6'),tag+' 74 montages dont six RAVE')
