...
def main():
 ...
 with sync_playwright() as p:
  ...
  for w,h in FORMATS:
   ...
   v(pg.evaluate('Object.keys(EUR_CAT).length===119 && new Set(EUR_ORDRE).size===119'),tag+' 119 modules uniques')
   v(pg.evaluate('EUR_MONTAGES.length===66 && EUR_MONTAGES.filter(p=>p.fam==="rave").length===6'),tag+' 66 montages dont six RAVE')
