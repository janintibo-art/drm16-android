...
def main():
 ...
 with sync_playwright() as p:
  ...
  for w,h in FORMATS:
   ...
   v(pg.evaluate('Object.keys(EUR_CAT).length===113 && new Set(EUR_ORDRE).size===113'),tag+' 113 modules uniques')
   v(pg.evaluate('EUR_MONTAGES.length===56 && EUR_MONTAGES.filter(p=>p.fam==="rave").length===6'),tag+' 56 montages dont six RAVE')
