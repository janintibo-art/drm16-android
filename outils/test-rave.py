...
def main():
 ...
 with sync_playwright() as p:
  ...
  for w,h in FORMATS:
   ...
   v(pg.evaluate('Object.keys(EUR_CAT).length===120 && new Set(EUR_ORDRE).size===120'),tag+' 120 modules uniques')
   v(pg.evaluate('EUR_MONTAGES.length===68 && EUR_MONTAGES.filter(p=>p.fam==="rave").length===6'),tag+' 68 montages dont six RAVE')
