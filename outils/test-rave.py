...
def main():
 ...
 with sync_playwright() as p:
  ...
  for w,h in FORMATS:
   ...
   v(pg.evaluate('Object.keys(EUR_CAT).length===116 && new Set(EUR_ORDRE).size===116'),tag+' 116 modules uniques')
   v(pg.evaluate('EUR_MONTAGES.length===60 && EUR_MONTAGES.filter(p=>p.fam==="rave").length===6'),tag+' 60 montages dont six RAVE')
