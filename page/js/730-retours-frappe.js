/* v270 — Témoins de départ des voix internes, pas de mesure sonore.
...
var RETOURS_FRAPPE=(function(){
  ...
  function programmer(m,c,t,k,note){
    ...
    var e={m:m,t:Math.max(maintenant,t),k:k,note:note,sig:sens(m)};
    /* Les deux voix d'un unisson représentent une seule frappe. */
    if(file.some(function(a){return a.m===m && a.k===k && a.note===note && Math.abs(a.t-e.t)<.0001;}))return;
  ...
  return {programmer:programmer,effacer:effacer,synchroniser:synchroniser,
    inspecter:function(){return {version:270,mode:mode,masque:masque,file:file.length,impacts:impacts.size,appuis:appuis.size,
